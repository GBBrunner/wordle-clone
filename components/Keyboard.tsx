      ...
      {ROWS.map((row, ri) => (
        <View key={ri} style={styles.kbRow}>
          {row.map((key) => {
            ...
            return (
              <Pressable key={key} onPress={() => onKey(key)} style={[styles.key, state && styles[state]]}>
...
const styles = StyleSheet.create({
  ...
  key: { paddingHorizontal: 10, paddingVertical: 12, borderRadius: 6, backgroundColor: '#818384' },
